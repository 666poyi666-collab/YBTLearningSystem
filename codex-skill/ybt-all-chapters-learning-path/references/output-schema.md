...
- A learner-facing page may keep browser-local stars, listened cycles, passed items, questions, and a `复制进度` action. It must state that the copied browser state is not cloud progress until a confirmed `math_sync_progress_snapshot` succeeds. Confirmed errors/classifications are written by `math_record_wrong_question`; live wrong-question documents come from `math_export_wrong_questions`; deferred cycles remain distinct from completion. The supplementary practice book is optional and cannot block YBT/course/section/chapter completion.
