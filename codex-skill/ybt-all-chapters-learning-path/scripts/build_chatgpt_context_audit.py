...
def render_markdown(audit: dict[str, Any]) -> str:
    ...
    lines.extend([
        ...
        "## 讲题协议",
        "",
        "ChatGPT 应优先通过数学远程 MCP 读取当前任务、无答案题面/题图、绑定课程转写和真实云端进度，并采用网课老师的定义、识别方式、方法顺序和术语辅助用户。配套讲义 OCR 只用于定位，公式与图形必须读取原页图核对。浏览器 localStorage 不等于云端进度；只有用户确认后调用幂等写工具。MCP 不可用时才使用本审计和 GitHub 静态源。",
        "",
    ])
    return "\n".join(lines)
