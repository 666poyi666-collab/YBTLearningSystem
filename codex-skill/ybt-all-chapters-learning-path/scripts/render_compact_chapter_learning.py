...
(() => {
  ...
  function helpPrompt(panel) {
    ...
    return `你是“数学选择性必修一”项目里的高中数学学习辅助老师。请优先使用 GitHub 连接读取仓库 666poyi666-collab/ybt-learning-system-v7 中对应章节的题包、课程转写和学习路径；8.5 对话文件只作为辅助经验参考，不作为本节事实来源。\n\n当前循环：${title}\n先听课程：${course}\n做题顺序：${order}\n我的疑问：${question}\n\n请按以下格式回答：\n1. 当前卡点属于概念、方法入口、计算、图形识别还是书写；\n2. 只给一个最小提示，不直接公布结果；\n3. 让我先提交自己的第一步或完整尝试；\n4. 说明下一步只做哪一道题或哪一个动作；\n5. 如果仓库或项目资料没有证据，明确说“资料不足”，不要猜。\n不要展开五人格压力测试，不要输出内部 ID、答案侧车或长篇课程总表。`;
  }
  ...
  function progressPrompt() {
    ...
    return `请使用 @GitHub 读取仓库 666poyi666-collab/ybt-learning-system-v7，并先核对 data/chatgpt_context/chapter12_complete_audit.json。下面是本浏览器导出的实时学习进度快照，它只代表当前用户操作，不等于仓库证据账本或内部模拟进度。\n\n章节：${document.querySelector('.brand h1')?.textContent?.trim() || `第${chapter}章`}\n${sections.join('\n')}\n\n请据此定位当前下一步。讲题前必须读取当前题目的无答案题面和所有绑定课程转写正文，采用网课老师的定义、术语、识别方式和方法顺序；先看我的尝试，只给一个最小提示，不直接公布结果。`;
  }
