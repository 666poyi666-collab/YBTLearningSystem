...
- The 2026 practice book is optional and answer-free in this source. It never blocks course, YBT, section, or chapter completion. Its OCR is search evidence only; `math_get_practice_page` must return the original page image before any formula, diagram, or exact stem is treated as verified.
- Handwritten work is a separate evidence layer. First verify the question source, transcribe each line, identify exactly one first divergence when possible, mark later lines as downstream contamination when appropriate, and save a `proposed` analysis with normalized boxes and LaTeX through `math_record_handwriting_analysis`. Generate the visible review as an unchanged original image plus transparent `fill=none` SVG overlays; only explicit learner confirmation can promote it to `math_record_wrong_question`.
